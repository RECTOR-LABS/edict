...
- The form posts to a **Route Handler**: `<form action="/api/auth/request-link" method="POST">`. **No client-side state, no Alpine.js, no `useFormStatus`.** Success message is always visible below the form (the spec's enumeration-defense model — we don't branch on email match). If the user double-submits, that's fine: the handler is idempotent-enough (one token per match each call; Phase D already accepted token accumulation as a trade-off for CLI simplicity, same reasoning applies here).
  - **Migration note (2026-05-30):** this originally shipped as a direct Server Action (`<form action={requestMagicLinkAction}>`). During the Vercel migration it was rewired to a thin Route Handler that calls the same `requestMagicLinkAction` — the action logic is unchanged; only the form target moved. This sidesteps a Next 16 Server-Action streaming bug on Vercel. The historical references to a "Server Action `<form action={…}>`" below describe that original port.
